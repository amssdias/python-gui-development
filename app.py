...
# Get info for save or unsaved tabs
text_contents = dict()
...
def create_file(content="", title="Untitled"):
    ...
    text_area["vscrollcommand"] = text_scroll.set
...
def confirm_quit():
    ...
    for tab in notebook.tabs():
        ...
        # If one tab content is not equal to dict, then break
        if hash(content) != text_contents[str(text_widget)]:
            unsaved = True
            break

    # Ask user if they really want to close
...
def save_file():
    ...
    notebook.tab("current", text=filename) # Change name of current widget
